...
# BERT version of RobertaForSequenceClassificationDASCL
# For that reason, not all lines are commented, but basic comments are provided where the logic differs from RoBERTa 
class BertForSequenceClassificationDASCL(BertForSequenceClassification):
    """
    ...
    Attributes:
    config (['BertConfig']):  Model configuration class with all the parameters of the model. Initializing with a 
    ...
    def forward(
        self,
        input_ids_og: Optional[torch.Tensor] = None,
        attention_mask_og: Optional[torch.Tensor] = None,
        token_type_ids_og: Optional[torch.Tensor] = None,
        position_ids_og: Optional[torch.Tensor] = None,
        input_ids_kw: Optional[torch.Tensor] = None,
        attention_mask_kw: Optional[torch.Tensor] = None,
        token_type_ids_kw: Optional[torch.Tensor] = None,
        position_ids_kw: Optional[torch.Tensor] = None,
        ...
        token_type_ids_og (`torch.LongTensor` of shape `({0})`, *optional*):
            Segment token indices to indicate first and second portions of the inputs using the original text. Indices are selected in `[0, 1]`:
        ...
        attention_mask_kw (`torch.FloatTensor` of shape `({0})`, *optional*):
            ...
            - 0 for tokens that are **masked**.
            [What are attention masks?](../glossary#attention-mask)
        token_type_ids_kw (`torch.LongTensor` of shape `({0})`, *optional*):
            Segment token indices to indicate first and second portions of the inputs using the keyword-simplified text. Indices are selected in `[0, 1]`:
            - 0 corresponds to a *sentence A* token,
            - 1 corresponds to a *sentence B* token.
        ...
        outputs_og = self.bert(
            input_ids_og,
            attention_mask=attention_mask_og,
            token_type_ids=token_type_ids_og,
            position_ids=position_ids_og,
            head_mask=head_mask,
        ...
        if input_ids_kw is not None: 
            outputs_kw = self.bert(
                input_ids_kw,
                attention_mask=attention_mask_kw,
                token_type_ids=token_type_ids_kw,
                position_ids=position_ids_kw
            )
        
        # We use the pooled output from BERT
        pooled_output_og = outputs_og[1]
        if input_ids_kw is not None:
            pooled_output_kw = outputs_kw[1]

        # this dropout step before getting logits is in the original BERT code 
        pooled_output_og = self.dropout(pooled_output_og)
        logits_og = self.classifier(pooled_output_og)

        # repeat the previous dropout step but using the keyword-simplified text
        if input_ids_kw is not None: 
            pooled_output_kw = self.dropout(pooled_output_kw) 
        # if data augmentation is used, get the logits for the keyword-simplified text 
        if self.data_augmentation: 
            logits_kw = self.classifier(pooled_output_kw) 
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                if self.num_labels == 1:
                    ...
                    if self.data_augmentation: 
                        loss_kw = loss_fct(logits_kw.squeeze(), labels.squeeze())
                else:
                    loss_mod = loss_fct(logits_og, labels)
                    if self.data_augmentation: 
                        loss_kw = loss_fct(logits_kw, labels) 
        ...
        if input_ids_kw is not None:
            ...
            if self.contrastive: 
                po = self.proj(pooled_output_og)
                pk = self.proj(pooled_output_kw)
        ...
        # This is equivalent to vanilla BERT
        else:
            loss = loss_mod 
    ...
    # This function calculates DASCL 
